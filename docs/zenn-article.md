...
使い方をレクチャーしても、いかに業務を効率化できるかを伝えても、格差は広がるばかりです。

この課題を解決したい。そして「業務に対する AI の活用方法」という暗黙知を形式知に変え、会社に浸透させたい。その想いで、Microsoft Agent Hackathon 2026 に向けて開発したのが『Kodama』です。
...
と、まっさらな AI を渡されるのは、実は多くの人にとって心理的ハードルが高いお願いなのではないでしょうか。

そこで Kodama が目指すのは、「みんなが育てた AI」です。

オフィスの給湯室で交わされる「それ、こうやったらすぐ終わったよ」という何気ない立ち話。あの自然な知見の共有に、システムでレバレッジをかける。

ゼロから AI の使い方を考えるのではなく、先輩や同僚が試行錯誤して見つけたコツを、ただなぞってみるだけでいい。AI 活用の最初の一歩をそっと後押しする。それが Kodama です。
...
| 外部連携 | Slack（Socket Mode / Bolt） | `#はてなボックス` の受動検知 |
...
今回のハッカソンでは、「手動登録＋セマンティック検索からの提案」に加え、Slack `#はてなボックス` を通じた **受動的な困りごと検知** までを MVP として実装しました。今後は Microsoft Graph を通じたより広範な自動検知や、Power Automate 連携まで広げていく予定です。
