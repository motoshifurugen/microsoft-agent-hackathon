...
| F-3     | マッチングと伝播       | 困りごとデータ + 成功事例 DB + 組織グラフ               | 関連度スコアリング (決定論) + 伝播戦略選択 (LLM)     | Teams 通知 / 会議自動セット                                     |
...
**Step 1. 組織関係グラフ (MVP は 2 グラフ):**

| グラフ           | データソース             | 用途                 | MVP     |
| ---------------- | ------------------------ | -------------------- | ------- |
| 公式組織図       | Entra ID                 | 部署・役職・上長関係 | 対応    |
| ドキュメント著者 | SharePoint               | 文書作成者の特定     | 対応    |
| コラボグラフ     | Teams / Outlook 通信頻度 | 実際の協業関係       | Phase 2 |
| 負荷グラフ       | Calendar 密度            | 現在の余裕度         | Phase 2 |

**Step 2. 関連度スコアリング (決定論):**

候補上位 N 件を以下の式で抽出する。重み α 〜 ε の初期値は実装時に決定する。

```
score = α * 業務文脈類似度 (embedding)
      + β * 人的距離 (1 / コラボ頻度 + 0.1)
      + γ * 過去案件の成功率
      + δ * (1 - 負荷)
```
