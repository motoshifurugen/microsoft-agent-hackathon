"""Slack 投稿を支援候補シグナルに変換して保存する検知サービス。

Slack Bot (src/slack/bot.py) と デモ用 API (POST /api/slack/mock) の共通入口。
...
def handle_message(
    *,
    ...
    signal = replace(
        signal, kodama_url=build_kodama_url(base_url, result.business_category, signal.id)
    )
