...
from src.tools.cosmos_io import get_all_embeddings, get_all_success_cases, has_embeddings
...
def _snippet_for(case_id: str) -> str:
    raw = get_all_success_cases().get(case_id, {})
...
def _embedding_search(text: str, top_k: int, exclude_user_id: str | None) -> list[SearchHit]:
    ...
    for case_id, vec in get_all_embeddings().items():
...
def _string_match_search(text: str, top_k: int, exclude_user_id: str | None) -> list[SearchHit]:
    needle = text.lower()
    scored: list[SearchHit] = []
    for case_id, case in get_all_success_cases().items():
        if _should_exclude(case, exclude_user_id):
            continue
...
def semantic_search(
    ...
    if has_embeddings():
        try:
            ...
            _logger.warning("embedding search failed, falling back to string match: %s", exc)
